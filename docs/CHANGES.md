...
- fused QKV 路径
- 调试基建：`GGML_MM_PHASE=1` 逐阶段计时；`GGML_SCHED_PROFILE`/`GGML_SCHED_TIMING`（分 split 输入/图耗时与 label）输出改走 stderr（llama-bench 静音 GGML_LOG 后仍可见）；`GGML_REPACK_IQ2_XXS=0` 单独关 IQ2_XXS repack；`LLAMA_DECODE_TIMING=1` 逐 ubatch `[ubatch-prof]` 分段计时、`GGML_OP_TIMING=1` 追加逐图 `[gc-prof]`（n_batch≤32）
...
- **op-offload 小图门控（2026-08-08，修复纯 CPU -ngl 0 decode 严重回归）**：sched 的 op_offload（把 host 权重 op 卸到 GPU 算）按设计只划算于大 batch，但此前对任意 n_tokens 的图生效——dsv4 decode（1 token/ubatch）时每层 FLASH_ATTN 因 `nrows(output)≥32`（sinks 权重是 WEIGHTS src）赢得 offload 判定（cause `1.off`）成为 CUDA 种子，sched pass-2 扩张（`2.sup`）把邻接 CPU op 成片拉进 CUDA split，其中 wo_a 的 34MiB/层 host 权重被当作 split input **每 token 重拷**：sched profile 实测 151 splits/图、107 个 CUDA split、CPU→CUDA 1488.9MiB/token（~183ms），纯 CPU TG 仅 2.69~3.51。修复：`llama-context` 在 process_ubatch 按 `n_tokens >= 32`（对齐 `GGML_OP_OFFLOAD_MIN_BATCH` 默认值）用新 API `ggml_backend_sched_set_op_offload` 动态开关——PP（512 token）保留 GPU 助力，decode/小 ubatch（<32）整图留 CPU。实测（dsv4 284B IQ2_XXS，-ngl 0，72t，numa distribute，fa1，b2048/ub512，mmap0）：tg128 2.69/3.51 → **8.29±0.21**（`-nopo 1` 对照 8.89±0.05，二者机制等价；上游 e9fa0781f 默认参数同样带病，仅 5.04±0.07）；pp2048 = 117.6（修复前 ~126，op_offload 全关对照 95.5）——PP 助力基本保留。GPU 卸载路径（-ngl 99）不受影响：权重已在卡上时 `1.off` 前置条件（host buffer 权重）本就不成立
