...
=======
- **DSV4 q1 decode indexed sparse FA 默认化 + q8 KV 兼容（2026-08-06）**：`LLAMA_DSV4_FUSED_INDEXED_FA` 默认 `=2`（仅 q1 decode 开），raw/compressed 两段 KV 不再每步全宽 concat；F16 KV 专用（sparse kernel 经 row map 直接寻址 F16 backing）。量化 KV（q8_0 等）q1 decode 默认走 compact gather 路径（`LLAMA_DSV4_Q8_SPARSE_FA=1`）：`get_rows` 仅物化选中的 compressed 行（CUDA 新增同类型量化 dst 块拷贝 gather；CPU 原生消费量化行），dense MMA 只对 compact concat 物化 F16 scratch；`=0` 回落 dense top-k mask 全扫。CUDA `supported()` 兜底：非 2kv sparse op + 量化 KV 时对**全宽物理 backing** 物化 F16（`K->view_src`），任何组合都留在 GPU MMA，绝不回落 CPU backend。数值口径：sparse 本非 bit-exact，gather-dequant 顺序对拍 bit-exact（`test-dsv4-gather-dequant`），验收为指纹对拍
