...
#pragma unroll
            ...
            if (threadIdx.x == i && (rows_per_cuda_block == 1 || uint32_t(row0 + i) < stride_col_dst)) {
                ...
                if constexpr (has_fusion) {
                    ...
                    result += x_biases[j];
                    result = fminf(fmaxf(result, fusion.x_clamp_min), fusion.x_clamp_max);
                    if (use_gate) {
                        ...
                        gate_value = fminf(fmaxf(gate_value, fusion.gate_clamp_min), fusion.gate_clamp_max);
                        switch (active_glu) {
...
static __global__ void mul_mat_vec_q_moe(
    ...
    const int32_t * GGML_CUDA_RESTRICT ids = ids_ptr;
    float         * GGML_CUDA_RESTRICT dst = dst_ptr;
    const void    * GGML_CUDA_RESTRICT vgate = fusion.gate;
...
void ggml_cuda_mul_mat_vec_q(
    ...
    ggml_cuda_pool_alloc<char> src1_q8_1(ctx.pool(), ne13*ne12 * ne11*ne10_padded * sizeof(block_q8_1)/QK8_1);
    {
        ...
        static const bool hot_cpu_q8 = []() {
            const char * value = getenv("GGML_CUDA_HOT_MXFP4_CPU_Q8");
            return value != nullptr && value[0] != '\0' && strcmp(value, "0") != 0;
        }();
        const bool hot_tensor = strncmp(src0->name, "hot_expert.", 11) == 0;
        const bool cpu_q8 = src0->type == GGML_TYPE_MXFP4 && hot_cpu_q8 && hot_tensor;
        if (cpu_q8 && hot_tensor) {
            static std::once_flag reported;
            std::call_once(reported, [&]() {
                std::fprintf(stderr, "[hot-q8] CPU-compatible MXFP4 activation codes enabled tensor=%s\n", src0->name);
            });
        }
        quantize_row_q8_1_cuda(src1_d, nullptr, src1_q8_1.get(), src0->type,
            ne10, s11, s12, s13, ne10_padded, ne11, ne12, ne13, cpu_q8, stream);
    }
