...
Q2/Q3/Q4 Phase A v1 selected `61/58/10`, with a **108.4046 GiB** dry run and proxy loss **0.898640× all-Q3**, but materialization was rejected when anomalous MXFP4 source data in the `blk.21` gate/up pair made a Q3 fp16 scale become `inf`. v2 neither clamps nor silently changes those values; it preserves the complete gate/up atomic pair as MXFP4. The full model has now been produced at **108.8159 GiB**, and all 129 expert tensors, the `Q2_K/Q3_K/Q4_K/MXFP4 = 61/56/10/2` counts, and the SHA256 gate passed. Reconstruction and traffic proxies are not quality evidence, however: PPL and raw TG/PP validation are still pending, so the model has no placeholder performance row and is not claimed to outperform Q3.

With the same CLI/DSO, MXFP4 DSpark draft, prompt/seed, CUDA placement, and `NMAX=2,p_min=0`, a Q3_K_XL -> corrected UDNL_MX run followed by the reverse order measured Q3 at **29.37/29.17** and UDNL at **33.11/33.64 tok/s**: means of 29.270 and **33.375 tok/s (+14.02%)**. Each format's generated-response hash and acceptance counts are stable across both orders, while UDNL acceptance is actually lower (63.556% versus 68.056%). This is evidence for the specified DSpark configuration, not a same-quality result: the established PPL differs (4.6047 versus 4.0189), and the formats generate different text.
