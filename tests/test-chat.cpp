...
static void test_deepseek_v4_thinking_retention() {
    ...
    auto render_preserve = [&](const std::vector<common_chat_msg> & messages, bool preserve_reasoning) {
        common_chat_templates_inputs inputs;
        inputs.messages = messages;
        inputs.add_generation_prompt = false;
        inputs.chat_template_kwargs["thinking"] = "true";
    ...
    // The generic preserve_reasoning option must control the template-specific flag.
    {
        auto prompt = render_preserve({ user_q1, asst_a1, user_q2, asst_a2 }, false);
        assert_not_contains(prompt, think_a1);
        assert_contains(prompt, think_a2);
    }
}
...
static void test_deepseek_v4_tool_result_ordering() {
    ...
    // Unknown result ids stay after results that can be matched to a call.
    {
        auto unknown_weather = weather_result;
        unknown_weather.tool_call_id = "unknown";
        auto prompt = render({ user_q, assist_calls, unknown_weather, time_result });
        assert_contains(prompt, "<tool_result>12:00</tool_result>\n\n<tool_result>sunny</tool_result>");
    }

    // A later assistant message invalidates the preceding tool-call ordering.
    {
        auto plain_assist = simple_assist_msg("No tools");
        auto prompt = render({ user_q, assist_calls, plain_assist, weather_result, time_result });
        assert_contains(prompt, "<tool_result>sunny</tool_result>\n\n<tool_result>12:00</tool_result>");
    }
}
