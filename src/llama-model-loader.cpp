...
struct ggml_tensor * llama_model_loader::create_tensor(
    ...
    auto buft_for_tensor = [&](ggml_tensor * t_meta) -> ggml_backend_buffer_type_t {
        ...
        // instrumentation (GGML_OFFLOAD_TRACE=1): why an _exps tensor is/isn't upgraded
        if (getenv("GGML_OFFLOAD_TRACE") != nullptr && strstr(t_meta->name, "_exps") != nullptr) {
            auto * gpu_dev_t = ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_GPU);
            ggml_backend_buffer_type_t hb = gpu_dev_t ? ggml_backend_dev_host_buffer_type(gpu_dev_t) : nullptr;
            fprintf(stderr, "[moe-pp-buft] %s ne2=%lld buft=%s from_override=%d is_host=%d env=%lld gpu=%p host_buft=%p(%s)\n",
                    t_meta->name, (long long) t_meta->ne[2], ggml_backend_buft_name(buft),
                    (int) buft_from_override, (int) ggml_backend_buft_is_host(buft), (long long) moe_pp_min_tokens,
                    (void *) gpu_dev_t, (void *) hb, hb ? ggml_backend_buft_name(hb) : "null");
        }
        ...
        if (getenv("GGML_OFFLOAD_TRACE") != nullptr && strstr(t_meta->name, "_exps") != nullptr) {
            fprintf(stderr, "[moe-pp-buft] AFTER %s buft=%s\n", t_meta->name, ggml_backend_buft_name(buft));
        }
