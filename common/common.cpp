...
common_init_result::common_init_result(common_params & params, bool model_only) :
    pimpl(new impl{}) {
    // apply --numa before the model is loaded (NUMA weight mirroring needs to be active already
    // at load time, e.g. it implies --no-mmap). llama_backend_init/llama_numa_init are idempotent
    // for frontends that already called them.
    llama_backend_init();
    if (params.numa != GGML_NUMA_STRATEGY_DISABLED) {
        ...
        if (params.numa == GGML_NUMA_STRATEGY_MIRROR) {
            llama_numa_set_mirror(params.numa_mirror);
        }
    }
...
struct llama_model_params common_model_params_to_llama(common_params & params) {
    ...
    if (params.tensor_buft_overrides.empty()) {
        ...
        // move overrides to device buffer types in front of the CPU overrides generated by
        // --cpu-moe/--n-cpu-moe: the model loader applies the first matching override, so
        // without this reordering the -ncmoe CPU rules (appended when --n-cpu-moe is parsed
        // before -ot) shadow any user -ot overrides to GPU buffer types.
        // llama-bench already merges its overrides in this order (user -ot first, ncmoe last).
        auto & tbo = params.tensor_buft_overrides;
        const auto term = std::find_if(tbo.begin(), tbo.end(),
            [](const llama_model_tensor_buft_override & o) { return o.pattern == nullptr; });
        std::stable_partition(tbo.begin(), term,
            [](const llama_model_tensor_buft_override & o) { return o.buft != ggml_backend_cpu_buffer_type(); });
        mparams.tensor_buft_overrides = tbo.data();
    }
